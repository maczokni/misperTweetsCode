# Make the df needed to build figure 2
#
...
# photo quality and retweeting broken down by gender and ethnic appearance.
#' @export



makeFig2Df <- function(misper_tweets) {
  fig_df_1 <- misper_tweets %>%
    dplyr::filter(image_quality_coded != "idk") %>%
  ...
  #order image quality
  fig_df$image_quality_coded <- factor(fig_df$image_quality_coded, levels = c("bad", "average", "good/excellent"))
